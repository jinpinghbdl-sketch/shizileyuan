...
| 语音识别 | Web Speech API（`SpeechRecognition`）| 单次识别模式（v6），适配华为/Android 平板 |
...
在 GitHub 新建仓库（如 `zishi-leyuan`），上传 `github-deploy/` 内的**全部文件**：

```
仓库根目录/
├── index.html          ← 主应用（v6 修复版，含华为平板语音识别修复）
├── blocks-data.json    字库数据
├── manifest.json       PWA 配置
...
**continuous 模式**：v6 改为 `continuous=false` 单次识别，每次说完自动结束，用户再点一次继续。适配华为/鸿蒙/Android 平板。v5 旧版使用 continuous=true 连续识别模式，适合桌面浏览器。
